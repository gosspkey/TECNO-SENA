...
$query = "SELECT CodEquipo, Luces FROM Luces";
$stmt = $db->prepare($query);
$stmt->execute();

$luces = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
<body>
    ...
    <form action="../../controlador/reservacontins.php" method="POST" class="container">
        <h2 class="titulo text-center"><strong>Reserva de Luces (Instructores)</strong></h2>
        <div class="row">
            <div class="col-md-6">
            <img src="../img/Luz650.png" class="img-fluid" alt="" width="300px">
            </div>
            ...
            <input type="hidden" name="tipoEquipo" value="luces">

            <label for="CodLuces" class="mr-2">¿Qué luces deseas reservar?</label>
                <select name="CodLuces" id="CodLuces" class="form-control" required>
                    <option value="">Seleccione una opción</option>
                    <?php foreach ($luces as $luces): ?>
                        <option value="<?php echo $luces['CodEquipo']; ?>">
                            <?php echo $luces['Luces']; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>    
