...
$query = "SELECT CodEquipo, Proyectores FROM Proyectores";
$stmt = $db->prepare($query);
$stmt->execute();

$proyector = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
<body>
    ...
    <form action="../../controlador/reservacontins.php" method="POST" class="container">
        <h2 class="titulo text-center"><strong>Reserva de Proyectores (Instructores)</strong></h2>
        <div class="row">
            <div class="col-md-6">
            <img src="../img/proyector-powerpro.png" class="img-fluid" alt="" width="500px">
            </div>
            ...
            <input type="hidden" name="tipoEquipo" value="proyector">

            <label for="CodProyector" class="mr-2">¿Qué proyector deseas reservar?</label>
                <select name="CodProyector" id="CodProyector" class="form-control" required>
                    <option value="">Seleccione una opción</option>
                    <?php foreach ($proyector as $proyector): ?>
                        <option value="<?php echo $proyector['CodEquipo']; ?>">
                            <?php echo $proyector['Proyectores']; ?>
                        </option>
                    <?php endforeach;
                     ?>
                </select>
