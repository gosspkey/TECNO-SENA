...
$query = "SELECT CodEquipo, Sonido FROM Sonido";
$stmt = $db->prepare($query);
$stmt->execute();

$sonido = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
<body>
    ...
    <form action="../../controlador/reservacontins.php" method="POST" class="container">
        <h2 class="titulo text-center"><strong>Reserva de Equipos de sonido (Instructores)</strong></h2>
        <div class="row">
            <div class="col-md-6">
            <img src="../img/consolaUSD.png" class="img-fluid" alt="" width="300px">
            </div>
            ...
            <input type="hidden" name="tipoEquipo" value="sonido">

            <label for="CodSonido" class="mr-2">¿Qué equipo deseas reservar?</label>
                <select name="CodSonido" id="CodSonido" class="form-control" required>
                    <option value="">Seleccione una opción</option>
                    <?php foreach ($sonido as $sonido): ?>
                        <option value="<?php echo $sonido['CodEquipo']; ?>">
                            <?php echo $sonido['Sonido']; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>    
