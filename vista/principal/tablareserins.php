...
<?php
require_once('../../modelo/reservasins.php');
require_once('../../confi/conexion.php');
$database = new Database();
$db = $database->getConnection();
$Reservasins= new ReservasInstructores($db);
$Reservasins = $Reservasins->listarReservas();

if (!$Reservasins || $Reservasins->rowCount()==0){
    echo "<br>";
    echo "<div class='alert alert-warning'>No hay reservas registradas </div>";}else
{


        echo "<h1>Reservas Instructores</h1>";
        ...
        echo "<tbody>";
             while($f = $Reservasins->fetch(PDO::FETCH_ASSOC)){

               echo "<tr>
                    <td data-label='ID'>".$f["IDReserva"]. "</td>
                    <td data-label='IDInstructor'>".$f["Idins"]. "</td>
