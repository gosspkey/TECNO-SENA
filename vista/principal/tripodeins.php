...
$query = "SELECT CodEquipo, Tripodes FROM Tripodes";
$stmt = $db->prepare($query);
$stmt->execute();

$tripode = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
<body>
    ...
    <form action="../../controlador/reservacontins.php" method="POST" class="container">
        <h2 class="titulo text-center"><strong>Reserva de Tripodes (Instructores)</strong></h2>
        <div class="row">
            <div class="col-md-6">
            <img src="../img/Tripode290.png" class="img-fluid" alt="" width="300px">
            </div>
            ...
            <input type="hidden" name="tipoEquipo" value="tripode">

            <label for="CodTripode" class="mr-2">¿Qué tripode deseas reservar?</label>
                <select name="CodTripode" id="CodTripode" class="form-control" required>
                    <option value="">Seleccione una opción</option>
                    <?php foreach ($tripode as $tripode): ?>
                        <option value="<?php echo $tripode['CodEquipo']; ?>">
                            <?php echo $tripode['Tripodes']; ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>    
